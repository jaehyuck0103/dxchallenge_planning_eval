...
def run_episode(scenario, actor, jit_reset, jit_step_and_measure, jit_mask, rng):
    """Roll out one scenario and return the per-step metric arrays.

    The rollout stops at the episode end or as soon as a termination metric
    (collision / offroad) fires, mirroring the V-Max baseline evaluation.
    """
    state = jit_reset(scenario)

    rng, init_rng = jax.random.split(rng)
    actor_state = actor.init(init_rng, jit_mask(state))

    step_records = []
    while not bool(state.is_done):
        rng, step_rng = jax.random.split(rng)
        output = actor.select_action(None, jit_mask(state), actor_state, step_rng)
        ...
        if any(step_metrics[key] > 0 for key in TERMINATION_KEYS):
            break

    return {key: np.array([r[key] for r in step_records]) for key in step_records[0]}
...
def main():
    ...
    data_iter = dataloader.simulator_state_generator(
    ...
    if args.disable_future_masking:
        ...
        jit_mask = lambda state: state  # noqa: E731
    else:
        jit_mask = jax.jit(mask_future_information)

    def step_and_measure(state, action):
        new_state = env.step(state, action)
        return new_state, compute_step_metrics(new_state)

    jit_step_and_measure = jax.jit(step_and_measure)
    ...
    for scenario_index, scenario in enumerate(data_iter):
        if args.max_scenarios is not None and scenario_index >= args.max_scenarios:
            break

        rng, episode_rng = jax.random.split(rng)
        try:
            step_arrays = run_episode(
                scenario, actor, jit_reset, jit_step_and_measure, jit_mask, episode_rng
            )
            episode = challenge_metrics.episode_scores(step_arrays, TERMINATION_KEYS)
            episode["error"] = 0
        except Exception:
            print(f"-> ERROR in scenario {scenario_index} (scored 0):")
            traceback.print_exc()
            num_errors += 1
            episode = {
                "episode_length": 0,
        ...
        if (scenario_index + 1) % 50 == 0:
            ...
            print(
                f"-> {scenario_index + 1} scenarios | rideflux_score {mean_score:.4f} "
                f"| {elapsed / (scenario_index + 1):.2f}s per scenario"
            )
